...
export default async function SetterDetailPage({
  ...
  if (!setter || setter.role !== "SETTER") notFound();

  const commissionRate = setter.commissionRate ?? 0;
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <Link href="/compta/setters" className="text-gray-400 hover:text-gray-600 text-sm">
            ← Setters
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{setter.name}</h1>
            <p className="text-sm text-gray-400">
              {setter.email} · Taux commission :{" "}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <p className="text-sm text-gray-500">Commission setter</p>
      ...
      <div className="space-y-4">
              ...
              <div className="px-5 py-4 flex items-center justify-between border-b border-gray-100">
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    Closé le {formatDate(deal.closedAt)} · Closer : {deal.closer.name}
                  </p>
                </div>
                <div className="text-right">
        ...
        {setter.setterDeals.length === 0 && (
          <div className="bg-white rounded-xl border border-gray-200 p-10 text-center text-gray-400 text-sm">
            Aucun deal associé à ce setter.
          </div>
        )}
      </div>
    </div>
